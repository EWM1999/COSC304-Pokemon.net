...
class OrderDB:
    ...
    def listCustomerOrders(self, customerId):
        ...
        cursor = self.cnx.cursor()
        # TODO: Execute query and build output string
        query = "SELECT OrderId, OrderDate, CustomerId, EmployeeId, Total from Orders where CustomerId = %s"
...
#Total results: 4
    ...
    def computeOrderTotal(self, orderId):
        ...
        # TODO: Execute the query and return a cursor
        cursor = self.cnx.cursor(buffered=True)
        query = "SELECT SUM(Quantity * Price) AS orderTotal FROM OrderedProduct WHERE OrderId = %s GROUP BY (OrderId);"
        cursor.execute(query, (orderId,))
        return cursor
